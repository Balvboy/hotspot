...
BiasedLocking::Condition BiasedLocking::revoke_and_rebias(Handle obj, bool attempt_rebias, TRAPS) {
  ...
  // 1. 获取当前对象的markword
  markOop mark = obj->mark();
  // 2. is_biased_anonymously 这个函数判断的是，开启偏向锁，并且没有偏向到任何线程
  // 3. attempt_rebias 目前不知道是什么意思，字面意思是 尝试重新偏向，但是这里又一个取反，则意思是不尝试重新偏向

  // (mark->has_bias_pattern()&& mark->biased_locker() == NULL) && !attempt_rebias
  if (mark->is_biased_anonymously() && !attempt_rebias) {
    ...
    //4. 下面的代码的意思是，用原始的对象头CAS替换当前的markword
    // 但是问题是，根据上面的判断条件，这里应该是没有偏向才对
    // 如果，CAS设置成功，因为是不尝试重新偏向，所以直接返回偏向锁撤销成功
    ...
    if (!prototype_header->has_bias_pattern()) {
      ...
      //这里是偏向锁超时的逻辑
      //如果超时了，并且尝试重新偏向，则会返回 BIAS_REVOKED_AND_REBIASED，表示偏向到了新线程
      //如果不尝试，则返回 BIAS_REVOKED
