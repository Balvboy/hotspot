...
static BiasedLocking::Condition revoke_bias(oop obj, bool allow_rebias, bool is_bulk, JavaThread* requesting_thread) {
  ...
  if (biased_thread == NULL) {//如果没有偏向线程
    ...
    return BiasedLocking::BIAS_REVOKED;//返回偏向取消状态
  }

  // Handle case where the thread toward which the object was biased has exited
  bool thread_is_alive = false;//检测是否具有线程具有当前线程id
  ...
  // Check to see whether it currently owns the lock and, if so,write down the needed displaced headers to the thread's stack.
  // Otherwise, restore the object's header either to the unlocked  or unbiased state.
  //
  //
...
BiasedLocking::Condition BiasedLocking::revoke_and_rebias(Handle obj, bool attempt_rebias, TRAPS) {
  ...
  // We can revoke the biases of anonymously-biased objects efficiently enough that we should not cause these revocations to update the heuristics
  // because doing so may cause unwanted bulk revocations (which are expensive) to occur.
  //
  //
  markOop mark = obj->mark();//获取对象头
  if (mark->is_biased_anonymously() && !attempt_rebias) {//若线程是偏向所状态以及线程Id为空则返回真。
    // We are probably trying to revoke the bias of this object due to an identity hash code computation.
    // Try to revoke the bias without a safepoint. This is possible if we can successfully ompare-and-exchange an unbiased header into the mark word of the object,
    // cmeaning that no other thread has raced to acquire the bias of the object.
    //
	//
	//根据散列码的计算,我们可能撤销对象的偏向锁 不在安全点下尝试撤销偏向锁,如果我们可以在对象的对象头成功compare-and-exchange一个无偏向锁头,意味着没有其他线程竞争偏向锁
    markOop biased_value       = mark;
    markOop unbiased_prototype = markOopDesc::prototype()->set_age(mark->age());//设置为无偏向锁状态
    markOop res_mark = (markOop) Atomic::cmpxchg_ptr(unbiased_prototype, obj->mark_addr(), mark);
    if (res_mark == biased_value) {
      return BIAS_REVOKED;
    }
  } else if (mark->has_bias_pattern()) {//若线程是偏向所状态
    ...
    if (!prototype_header->has_bias_pattern()) {
      ...
      if (attempt_rebias) {
        ...
        markOop biased_value       = mark;//设置对象头线程id
        markOop rebiased_prototype = markOopDesc::encode((JavaThread*) THREAD, mark->age(), prototype_header->bias_epoch());
        markOop res_mark = (markOop) Atomic::cmpxchg_ptr(rebiased_prototype, obj->mark_addr(), mark);//mark为比较值  rebiased_prototype为修改值
        if (res_mark == biased_value) {//如果修改后的结果和原来结果一直
          return BIAS_REVOKED_AND_REBIASED;//返回偏向已经撤销并且重新偏向
        }
      } else {
        markOop biased_value       = mark;
        markOop unbiased_prototype = markOopDesc::prototype()->set_age(mark->age());//设置为无偏向锁状态
